...
# The values at indices 0, 1 and 2 are the timestamp, the count of visitors, and whether the visitors entered or exited the mall (0 for exit and 1 for entrance), respectively. Here’s an example of a data point: [ 1440084737, 4, 0 ].
...
# findBusiestPeriod that returns the time at which the mall reached its busiest moment last year. The return value is the timestamp, e.g. 1480640292.
...
def find_busiest_period(data):
    """Return the time stamp with most people INSIDE the mall."""
    count = 0
    max_count = 0
    max_ts = None
    d_len = len(data)
    ...
    for i in range(d_len - 1):
        ...
        if count > max_count:  # update max people if new max occur
            max_count = count
            max_ts = data[i][0]
    return max_ts
...
if __name__ == '__main__':
    ...
    print(data + 'can be called with data')
    print('expected result for data is ' + '1487901211')
